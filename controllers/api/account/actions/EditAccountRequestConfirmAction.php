...
class EditAccountRequestConfirmAction extends RestAction
{
    ...
    public function run()
    {
        ...
        if($confirmActionsService->confirmByCode($confirmActionsModel, $form->secret_key)) {
            ...
            $accountsService->changeUsername($account, $data['username']);
            $emailManageAccountsService = new AccountsEmailManageService();
            $emailManageAccountsService->createRequestEditEmail($account, $data['email']);
            return new $this->modelClass(
                true,
                Yii::t('package-accounts', 'Профиль изменен'),
                Yii::t('package-accounts', 'Данные вашего профиля были успешно изменены.')
            );
        }
